                    ...
                    <div class="card-body">
                        <form id="submitForm">
                            ...
                            <div class="mb-3">
                                ...
                                <input type="text" class="form-control @error('nip') is-invalid @enderror" id="nip" name="nip" value="{{ $pegawai->nip }}" />
                            ...
                            <div class="mb-3">
                                ...
                                <select id="id_pangkat" name="id_pangkat" class="form-select form-control @error('id_pangkat') is-invalid @enderror">
                                    @foreach ($pangkats as $pangkat)
                                    <option value="{{ $pangkat->id }}" {{ $pegawai->id_pangkat == $pangkat->id ? 'selected' : '' }}>
                                        {{ $pangkat->pangkat }}
                                    </option>
                                    @endforeach
                                </select>
                            ...
                            <div class="mb-3">
                                ...
                                <select id="id_jabatan" name="id_jabatan" class="form-select form-control @error('id_jabatan') is-invalid @enderror">
                                    @foreach ($jabatans as $jabatan)
                                    <option value="{{ $jabatan->id }}" {{ $pegawai->id_jabatan == $jabatan->id ? 'selected' : '' }}>
                                        {{ $jabatan->jabatan }}
                                    </option>
                                    @endforeach
                                </select>
...
<script>
    $(document).ready(function() {
        $('#submitForm').on('submit', function(event) {
            ...
            $.ajax({
                url: "{{ route('pegawai.update', $pegawai->id) }}",
                method: 'POST',
                data: formData,
                contentType: false,
                processData: false,
